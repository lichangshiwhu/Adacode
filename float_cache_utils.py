...
def _compute_adjusted_params(keys_exp_count, values_exp_count):
    return analysis_best_bits(keys_exp_count), analysis_best_bits(values_exp_count)
...
class float11Layer(DynamicLayer):
    def __init__(
        self,
        keys_right: Optional[int] = 0,
        values_right: Optional[int] = 0,
        keys_nbits: Optional[int] = 3,
        values_nbits: Optional[int] = 3,
        residual_length: int = 0,
        block_size: int = 128,
        layout: str = 'legacy',
        seq_tile_size: Optional[int] = None,
        auto_init_compress_params: bool = False,
    ):
        super().__init__()
        ...
        self.auto_init_compress_params = bool(auto_init_compress_params)
        self.compress_params_initialized = keys_right is not None and values_right is not None and keys_nbits is not None and values_nbits is not None
        if self.auto_init_compress_params and self.compress_params_initialized:
            raise ValueError('auto_init_compress_params expects compression parameters to be None.')
        if not self.auto_init_compress_params and not self.compress_params_initialized:
            raise ValueError('Compression parameters must be provided unless auto_init_compress_params is enabled.')
        ...
        self.keys_update_proposals = 0
        self.values_update_proposals = 0
        ...
        self._pending_adjust_recompress_start_segment: Optional[int] = None
        self._pending_dense_block_seq_len = 0
        self._pending_dense_block_key_params: Optional[tuple[int, int]] = None
        self._pending_dense_block_value_params: Optional[tuple[int, int]] = None
        self._pending_dense_block_needs_adjust_params = False
        self._adjust_recompresses = 0
        self._delayed_adjust_block_flushes = 0
        self.auto_init_compress_param_updates = 0

    def _set_compress_params(
        self,
        keys_params: tuple[int, int],
        values_params: tuple[int, int],
        *,
        count_as_auto_init: bool = False,
    ) -> None:
        self.keys_right, self.keys_nbits = int(keys_params[0]), int(keys_params[1])
        self.values_right, self.values_nbits = int(values_params[0]), int(values_params[1])
        self.new_keys_right, self.new_keys_nbits = self.keys_right, self.keys_nbits
        self.new_values_right, self.new_values_nbits = self.values_right, self.values_nbits
        self.compress_params_initialized = True
        if count_as_auto_init:
            self.auto_init_compress_param_updates += 1

    def _ensure_compress_params_initialized(
        self,
        key_states: torch.Tensor,
        value_states: torch.Tensor,
        *,
        keys_exp_count: Optional[torch.Tensor] = None,
        values_exp_count: Optional[torch.Tensor] = None,
    ) -> tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        if self.compress_params_initialized:
            return keys_exp_count, values_exp_count
        if not self.auto_init_compress_params:
            raise RuntimeError('Compression parameters are not initialized.')
        if keys_exp_count is None:
            keys_exp_count = self._extract_exponent_histogram(key_states)
        if values_exp_count is None:
            values_exp_count = self._extract_exponent_histogram(value_states)
        if keys_exp_count is None or values_exp_count is None:
            raise RuntimeError('Unable to initialize compression parameters from empty KV tensors.')
        keys_params, values_params = _compute_adjusted_params(
            keys_exp_count.to('cpu'),
            values_exp_count.to('cpu'),
        )
        self._set_compress_params(keys_params, values_params, count_as_auto_init=True)
        return keys_exp_count, values_exp_count
    ...
    def _record_adjust_proposal(
        self,
        new_keys_params: tuple[int, int],
        new_values_params: tuple[int, int],
    ) -> tuple[bool, bool]:
        key_changed = (
            int(self.new_keys_right),
            int(self.new_keys_nbits),
        ) != (int(new_keys_params[0]), int(new_keys_params[1]))
        value_changed = (
            int(self.new_values_right),
            int(self.new_values_nbits),
        ) != (int(new_values_params[0]), int(new_values_params[1]))
        if key_changed:
            self.keys_update_proposals += 1
        if value_changed:
            self.values_update_proposals += 1
        return key_changed, value_changed

    def _record_effective_param_update(
        self,
        old_keys_params: tuple[int, int],
        old_values_params: tuple[int, int],
        new_keys_params: tuple[int, int],
        new_values_params: tuple[int, int],
    ) -> tuple[bool, bool]:
        key_changed = (
            int(old_keys_params[0]),
            int(old_keys_params[1]),
        ) != (int(new_keys_params[0]), int(new_keys_params[1]))
        value_changed = (
            int(old_values_params[0]),
            int(old_values_params[1]),
        ) != (int(new_values_params[0]), int(new_values_params[1]))
        if key_changed:
            self.keys_update_times += 1
        if value_changed:
            self.values_update_times += 1
        return key_changed, value_changed

    def adjust_compressor(self, keys_exp_count, values_exp_count):
        new_keys_params = analysis_best_bits(keys_exp_count)
        new_values_params = analysis_best_bits(values_exp_count)
        self._record_adjust_proposal(new_keys_params, new_values_params)
        self.new_keys_right, self.new_keys_nbits = new_keys_params
        self.new_values_right, self.new_values_nbits = new_values_params

    def has_pending_adjust_recompress(self) -> bool:
        return self._pending_adjust_recompress_start_segment is not None

    def has_pending_adjust_work(self) -> bool:
        return self._pending_adjust_recompress_start_segment is not None or self._pending_dense_block_needs_adjust_params

    def has_pending_dense_block(self) -> bool:
        return self._pending_dense_block_seq_len > 0

    def mark_adjust_recompress_start(self, start_segment: int) -> None:
        if self.layout != 'segmented_tiled':
            return
        start_segment = max(0, int(start_segment))
        if self._pending_adjust_recompress_start_segment is None:
            self._pending_adjust_recompress_start_segment = start_segment
        else:
            self._pending_adjust_recompress_start_segment = min(
                self._pending_adjust_recompress_start_segment,
                start_segment,
            )

    def mark_dense_block_pending(
        self,
        seq_len: int,
        key_params: tuple[int, int],
        value_params: tuple[int, int],
        needs_adjust_params: bool,
    ) -> None:
        if self.layout != 'segmented_tiled':
            return
        self._pending_dense_block_seq_len += int(seq_len)
        self._pending_dense_block_key_params = (int(key_params[0]), int(key_params[1]))
        self._pending_dense_block_value_params = (int(value_params[0]), int(value_params[1]))
        self._pending_dense_block_needs_adjust_params = bool(needs_adjust_params)
    ...
    def apply_pending_adjust_recompress_if_needed(
        ...
        new_values_right, new_values_nbits = values_params
        old_params = (
            self.keys_right,
            self.keys_nbits,
            self.values_right,
        ...
        new_params = (
            int(new_keys_right),
            int(new_keys_nbits),
            int(new_values_right),
            int(new_values_nbits),
        )
        self._record_adjust_proposal(
            (int(new_keys_right), int(new_keys_nbits)),
            (int(new_values_right), int(new_values_nbits)),
        )
        self.new_keys_right, self.new_keys_nbits = int(new_keys_right), int(new_keys_nbits)
        self.new_values_right, self.new_values_nbits = int(new_values_right), int(new_values_nbits)
        ...
        key_states = self._materialize_segments(
            self._compressed_prefix_key_segments[start_segment:]
        )
        value_states = self._materialize_segments(
            self._compressed_prefix_value_segments[start_segment:]
        )
        if key_states is None or value_states is None or key_states.shape[-2] == 0:
            return False
        ...
        self._flush_ready_blocks(need_adjust=False)
        if active_keys is not None and active_keys.shape[-2] > 0:
            delayed_keys, delayed_values = self._active_residual_tensors()
            if delayed_keys is None:
                self._set_active_residual_tensors(active_keys, active_values)
            else:
                self._set_active_residual_tensors(
                    _concat_seq_tensors(delayed_keys, active_keys),
                    _concat_seq_tensors(delayed_values, active_values),
                )
    ...
    def apply_pending_dense_block_if_needed(
        ...
        if self.layout != 'segmented_tiled':
            return False
        pending_len = int(self._pending_dense_block_seq_len)
        if pending_len <= 0:
            return False
        active_keys, active_values = self._active_residual_tensors()
        if active_keys is None or active_values is None or active_keys.shape[-2] < pending_len:
            return False

        if keys_params is None:
            keys_params = self._pending_dense_block_key_params
        if values_params is None:
            values_params = self._pending_dense_block_value_params
        if keys_params is None or values_params is None:
            return False
        self._pending_dense_block_seq_len = 0
        self._pending_dense_block_needs_adjust_params = False
        self._pending_dense_block_key_params = None
        self._pending_dense_block_value_params = None

        old_keys_params = (self.keys_right, self.keys_nbits)
        old_values_params = (self.values_right, self.values_nbits)
        self.new_keys_right, self.new_keys_nbits = int(keys_params[0]), int(keys_params[1])
        self.new_values_right, self.new_values_nbits = int(values_params[0]), int(values_params[1])
        self._record_adjust_proposal(
            (self.new_keys_right, self.new_keys_nbits),
            (self.new_values_right, self.new_values_nbits),
        )
        self.keys_right, self.keys_nbits = self.new_keys_right, self.new_keys_nbits
        self.values_right, self.values_nbits = self.new_values_right, self.new_values_nbits
        self._record_effective_param_update(
        ...
        block_keys = active_keys[..., :pending_len, :].contiguous()
        block_values = active_values[..., :pending_len, :].contiguous()
        remaining_keys = active_keys[..., pending_len:, :].contiguous()
        remaining_values = active_values[..., pending_len:, :].contiguous()
        if remaining_keys.shape[-2] == 0:
            remaining_keys = None
            remaining_values = None
        self._set_active_residual_tensors(block_keys, block_values)
        self._flush_ready_blocks(need_adjust=False)
        compressed_tail_keys, compressed_tail_values = self._active_residual_tensors()
        if compressed_tail_keys is not None and compressed_tail_keys.shape[-2] > 0:
            if remaining_keys is None:
                remaining_keys = compressed_tail_keys
                remaining_values = compressed_tail_values
            else:
                remaining_keys = _concat_seq_tensors(compressed_tail_keys, remaining_keys)
                remaining_values = _concat_seq_tensors(compressed_tail_values, remaining_values)
        self._set_active_residual_tensors(remaining_keys, remaining_values)
        self._delayed_adjust_block_flushes += 1
        return True
    ...
    @staticmethod
    def _extract_exponent_histogram(states: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
        if states is None or states.numel() == 0:
            return None
        if states.dtype != torch.bfloat16:
            states = states.to(torch.bfloat16)
        raw_i32 = states.contiguous().view(torch.uint16).to(torch.int32)
        exp_dense = ((raw_i32 >> 7) & 0xFF).to(torch.int64)
        return torch.bincount(exp_dense.reshape(-1), minlength=256).to(torch.int32)

    def _collect_adjust_hist_from_active_residual(self) -> tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        active_keys, active_values = self._active_residual_tensors()
        if active_keys is None or active_values is None:
            return None, None
        return (
            self._extract_exponent_histogram(active_keys),
            self._extract_exponent_histogram(active_values),
        )

    def _update_adjust_params_from_hist(
        self,
        keys_exp_count: Optional[torch.Tensor],
        values_exp_count: Optional[torch.Tensor],
    ) -> bool:
        if keys_exp_count is None or values_exp_count is None:
            return False
        new_keys_params, new_values_params = _compute_adjusted_params(
            keys_exp_count.to('cpu'),
            values_exp_count.to('cpu'),
        )
        if not self.compress_params_initialized:
            if not self.auto_init_compress_params:
                raise RuntimeError('Compression parameters are not initialized.')
            self._set_compress_params(new_keys_params, new_values_params, count_as_auto_init=True)
            return True
        self._record_adjust_proposal(new_keys_params, new_values_params)
        old_keys_params = (self.keys_right, self.keys_nbits)
        old_values_params = (self.values_right, self.values_nbits)
        self.new_keys_right, self.new_keys_nbits = int(new_keys_params[0]), int(new_keys_params[1])
        self.new_values_right, self.new_values_nbits = int(new_values_params[0]), int(new_values_params[1])
        if self.layout == 'segmented_tiled' and self.has_pending_dense_block():
            self._pending_dense_block_key_params = (self.new_keys_right, self.new_keys_nbits)
            self._pending_dense_block_value_params = (self.new_values_right, self.new_values_nbits)
            self._pending_dense_block_needs_adjust_params = False
            return True
        if self.layout == 'segmented_tiled':
            return True
    ...
    def _materialize_segments(self, segments: list[dict[str, Any]]) -> Optional[torch.Tensor]:
        if not segments:
            return None
        parts = [materialize_tiled_state(state) for state in segments]
        parts = [part for part in parts if part is not None and part.shape[-2] > 0]
        if not parts:
            return None
        if len(parts) == 1:
    ...
    def _compress_span(self, key_states: torch.Tensor, value_states: torch.Tensor, need_adjust: bool):
        if not self.compress_params_initialized:
            self._ensure_compress_params_initialized(key_states, value_states)
    ...
    def _stage_dense_block(self, collect_adjust_hist: bool):
        active_keys, active_values = self._active_residual_tensors()
        block_keys = active_keys[..., : self.block_size, :].contiguous()
        block_values = active_values[..., : self.block_size, :].contiguous()
        keys_exp_count = values_exp_count = None
        if collect_adjust_hist:
            _, _, _, _, keys_exp_count, values_exp_count = self._compress_span(
                block_keys,
                block_values,
                need_adjust=True,
            )
        self.mark_dense_block_pending(
            int(block_keys.shape[-2]),
            key_params=(self.keys_right, self.keys_nbits),
            value_params=(self.values_right, self.values_nbits),
            needs_adjust_params=collect_adjust_hist,
        )
        return keys_exp_count, values_exp_count
    ...
    def _flush_ready_blocks(
        self,
        need_adjust: bool,
        delay_segmented_block: bool = False,
        collect_adjust_hist: bool = False,
    ):
        keys_hist = None
        values_hist = None
        flushed_any = False
        active_keys, active_values = self._active_residual_tensors()
        if not self.compress_params_initialized and active_keys is not None and active_keys.shape[-2] >= self.block_size:
            self._ensure_compress_params_initialized(active_keys, active_values)
        while active_keys is not None and active_keys.shape[-2] >= self.block_size:
            if self.layout == 'segmented_tiled' and delay_segmented_block:
                block_keys_hist, block_values_hist = self._stage_dense_block(
                    collect_adjust_hist=collect_adjust_hist,
                )
                if block_keys_hist is not None:
                    keys_hist = block_keys_hist.clone() if keys_hist is None else keys_hist + block_keys_hist
                    values_hist = block_values_hist.clone() if values_hist is None else values_hist + block_values_hist
                break
            flushed_any = True
    ...
    def get_memory(self):
        ...
        return {
            ...
            'keys_update_times': self.keys_update_times,
            'values_update_times': self.values_update_times,
            'keys_update_proposals': self.keys_update_proposals,
            'values_update_proposals': self.values_update_proposals,
            ...
            'adjust_recompresses': self._adjust_recompresses,
            'delayed_adjust_block_flushes': self._delayed_adjust_block_flushes,
        }
    ...
    def update(
        ...
        if need_adjust and self.compress_params_initialized and not (self.layout == 'segmented_tiled' and self.has_pending_adjust_work()):
            self.keys_right, self.keys_nbits = self.new_keys_right, self.new_keys_nbits
            self.values_right, self.values_nbits = self.new_values_right, self.new_values_nbits
        ...
        keys_exp_count, values_exp_count, flushed_any = self._flush_ready_blocks(
            need_adjust=need_adjust,
            delay_segmented_block=(self.layout == 'segmented_tiled' and not prefill_fastpath),
            collect_adjust_hist=(self.layout == 'segmented_tiled' and need_adjust and not prefill_fastpath),
        )
        if self.layout == 'segmented_tiled' and need_adjust and keys_exp_count is None:
...
class Float11Cache(Cache):
    def __init__(
        ...
        auto_init_compress_params = bool(auto_init_compress_params)

        if auto_init_compress_params:
            if right is not None or exp_bits is not None:
                raise ValueError('auto_init_compress_params expects right and exp_bits to be None.')
            layers = [
                float11Layer(
                    None,
                    None,
                    ...
                    auto_init_compress_params=True,
                )
                for _ in range(config.num_hidden_layers)
            ]
        elif right is None or exp_bits is None:
            raise ValueError('right and exp_bits are required unless auto_init_compress_params is enabled.')
            ...
            layers = [
                float11Layer(
                    ...
                    layout=layout,
                    seq_tile_size=seq_tile_size,
                    auto_init_compress_params=False,
                )
                for _ in range(config.num_hidden_layers)
            ]
        else:
        ...
        self.layout = layout
        self.seq_tile_size = seq_tile_size if seq_tile_size is not None else block_size
        self.auto_init_compress_params = auto_init_compress_params
    ...
    def update(
        ...
        if self._segmented_adjust:
            ...
            if layer.has_pending_adjust_recompress():
                layer.apply_pending_adjust_recompress_if_needed(
                    (layer.new_keys_right, layer.new_keys_nbits),
                    (layer.new_values_right, layer.new_values_nbits),
                )
            elif layer.has_pending_dense_block() and not layer.has_pending_adjust_work():
    ...
    def get_memory(self):
        total_info = {
            ...
            'delayed_adjust_block_flushes': 0,
        }
        for layer in self.layers:
            ...
            total_info['keys_update_times'] += layer_mem['keys_update_times']
            total_info['values_update_times'] += layer_mem['values_update_times']
            total_info['keys_update_proposals'] += layer_mem['keys_update_proposals']
            total_info['values_update_proposals'] += layer_mem['values_update_proposals']
            ...
            total_info['adjust_recompresses'] += layer_mem['adjust_recompresses']
            total_info['delayed_adjust_block_flushes'] += layer_mem['delayed_adjust_block_flushes']
    ...
    def get_compress_params(self):
        return {
            'kv_auto_init_compress_params': bool(self.auto_init_compress_params),
            'kv_compress_params_initialized_layers': sum(
                int(layer.compress_params_initialized) for layer in self.layers
            ),
            'kv_auto_init_compress_param_updates': sum(
                int(layer.auto_init_compress_param_updates) for layer in self.layers
            ),
            'kv_current_rights': [
                None if not layer.compress_params_initialized else (int(layer.keys_right), int(layer.values_right))
                for layer in self.layers
            ],
            'kv_current_nbits': [
                None if not layer.compress_params_initialized else (int(layer.keys_nbits), int(layer.values_nbits))
                for layer in self.layers
            ],
        }
    ...
    def synchronize_and_release(self):
        ...
        for layer in self.layers:
            ...
            layer._pending_adjust_recompress_start_segment = None
            layer._pending_dense_block_seq_len = 0
            layer._pending_dense_block_key_params = None
            layer._pending_dense_block_value_params = None
            layer._pending_dense_block_needs_adjust_params = False
            layer._adjust_recompresses = 0
            layer._delayed_adjust_block_flushes = 0
